...
class CGFXEngine(object):
	...
	def mainInit(self, width, height, fullscreen, noframe, load_screen= True):
		...
		if fullscreen == True:
			...
			self.screen = pygame.display.set_mode((width, height), HWSURFACE|DOUBLEBUF|NOFRAME)
		self.images = {}
		if load_screen == True:
		...
		for i in SPQR.GRAPHICS_D:
			files.extend(["../data/" + i +"/" + name for name in os.listdir("../data/" + i + "/")])
	...
	def checkInputs(self):
		...
		if event.type != NOEVENT:
			# if it's a rmb down, then possibly exit ## why this possibility ? I thought it was a bug ! 
		...
		for bar in self.windows[-1].items:
			if bar.active == True and bar.wtype == SPQR.WT_BUTTON:
				...
				if bar.highlight == True and bar.wtype == SPQR.WT_BUTTON:
					...
					dest = pygame.Rect(bar.rect.x+self.windows[-1].rect.x,
						bar.rect.y+self.windows[-1].rect.y, bar.rect.w, bar.rect.h)
					self.screen.blit(bar.image, (dest.x, dest.y))
					pygame.display.update(dest)
					return True
		return False
	...
	def mapClick(self, x, y):
		...
		region = SDATA.regionClicked(x, y)
		if region != False:
			self.renderRegionInfoBox(region)
			self.renderImageUnits(region)
			self.updateGUI()
		else:
	...
	# this is the function that allows you to pan the map with the
	# middle mouse button
	def panMap(self):
	...
	def renderImageUnits(self, region):
		...
		xpos = int((SPQR.SCREEN_WIDTH - xpos) / 1.5)
		ypos = SPQR.SCREEN_HEIGHT - SPQR.UNIT_HEIGHT *2
