...
class CSPQR(object):
	def __init__(self):
		...
		self.noframe = SPQR.NOFRAME
		self.intro = True
		self.init_only = False
		# init the data
		SDATA.addUnits()
		self.sortOptions()
		SGFX.gui.mainInit(SPQR.SCREEN_WIDTH, SPQR.SCREEN_HEIGHT, self.fullscreen, self.noframe)
	...
	def executeFlag(self, flag):
		...
		if flag == 'f':
			...
			print "  Copr. 2005-2017, released under the GNU Public License v2"
			print "  Last code update: " + SPQR.LAST_UPDATE
			print "  If you want to accomplish a personal challenge, help us to improve the game !"
			sys.exit(True)
		elif flag == 'g':
	...
	def setupStart(self):
		...
		menu[1].addChild(SMENU.CMenuChild("Visit Council", "senate", "F2", SEVENT.menuEmpireCouncil))
		menu[1].addChild(SMENU.CMenuChild("Inspect Armies", "military", "F3", SEVENT.menuEmpireMilitary))
		menu[2].addChild(SMENU.CMenuChild("About", "about", "Ctrl+A", SEVENT.menuHelpAbout))
		menu[2].addChild(SMENU.CMenuChild("Statistics", "statistics", "", SEVENT.menuHelpStatistics))
		...
		bwindow = SWINDOW.CWindow(0, 0, 
								  SPQR.SCREEN_WIDTH, SPQR.SCREEN_HEIGHT, "",
								  False, "map_widgets")
	
		## add board border		
		for i in range(SPQR.SCREEN_WIDTH / 14):
			w = SGFX.gui.iWidth('board_border')
			h = SGFX.gui.iHeight('board_border')
			y = SPQR.SCREEN_HEIGHT-175
			board_border = SWIDGET.CImage(i*14, y, w, h, 'board_border')
			bwindow.addWidget(board_border)
		
		# and the mini-map on the rhs
		w = SGFX.gui.iWidth("small_map")
		h = SGFX.gui.iHeight("small_map")
		x = SPQR.SCREEN_WIDTH - (w + 30)
		y = SPQR.SCREEN_HEIGHT - \
			(SGFX.gui.iHeight("small_map") + bwindow.rect.y +6)
		...
		## add small map background image
		w = SGFX.gui.iWidth('small_map_back')
		h = SGFX.gui.iHeight("small_map_back")
		x = SPQR.SCREEN_WIDTH - w 
		y = SPQR.SCREEN_HEIGHT - h
		small_map_back = SWIDGET.CImage(x, y, w, h, 'small_map_back')
		bwindow.addWidget(small_map_back)	

		## add region background image
		w = SGFX.gui.iWidth('region_back')
		h = SGFX.gui.iHeight("region_back")
		x = SPQR.SCREEN_WIDTH - w 
		y = SPQR.SCREEN_HEIGHT - h
		region_back = SWIDGET.CImage(0, y, w, h, 'region_back')
		bwindow.addWidget(region_back)

		## add year
		year = SWIDGET.buildLabel(text=str(SPQR.START_YEAR))
		year.rect.x = SPQR.SCREEN_WIDTH - 150
		year.rect.y = SPQR.SCREEN_HEIGHT - 160
		bwindow.addWidget(year)			

		## add tax image
		w = SGFX.gui.iWidth('tax')
		h = SGFX.gui.iHeight('tax')
		x = SPQR.SCREEN_WIDTH / 2 - (SGFX.gui.iWidth('tax') *2)
		y = SPQR.SCREEN_HEIGHT - 185
		tax = SWIDGET.CImage(x, y, w, h, 'tax')
		tax.callbacks.mouse_ldown = SEVENT.notYetCoded
		tax.active = True
		bwindow.addWidget(tax)		

		## add message image
		w = SGFX.gui.iWidth('message')
		h = SGFX.gui.iHeight('message')
		x = SPQR.SCREEN_WIDTH / 2 - (SGFX.gui.iWidth('message') *3 +20)
		y = SPQR.SCREEN_HEIGHT - 185
		message = SWIDGET.CImage(x, y, w, h, 'message')
		message.callbacks.mouse_ldown = SEVENT.notYetCoded
		message.active = True
		bwindow.addWidget(message)

		## add army image
		w = SGFX.gui.iWidth('army')
		h = SGFX.gui.iHeight('army')
		x = SPQR.SCREEN_WIDTH / 2 - (SGFX.gui.iWidth('army') -20)
		y = SPQR.SCREEN_HEIGHT - 185
		army = SWIDGET.CImage(x, y, w, h, 'army')
		army.callbacks.mouse_ldown = SEVENT.notYetCoded
		army.active = True
		bwindow.addWidget(army)

		## add time image
		w = SGFX.gui.iWidth('time')
		h = SGFX.gui.iHeight('time')
		x = SPQR.SCREEN_WIDTH - (w * 5)
		y = SPQR.SCREEN_HEIGHT - 185
		time = SWIDGET.CImage(x, y, w, h, 'time')
		time.callbacks.mouse_ldown = SEVENT.notYetCoded
		time.active = True
		bwindow.addWidget(time)

		## add background advisor image
		w = SGFX.gui.iWidth('back_advisor')
		h = SGFX.gui.iHeight('back_advisor')
		x = SPQR.SCREEN_WIDTH - w
		y = SPQR.MENU_Y_OFFSET
		back_advisor = SWIDGET.CImage(x, y, w, h, 'back_advisor')
		bwindow.addWidget(back_advisor)

		## add advisor image to the back
		w = SGFX.gui.iWidth('advisor_stratmap')
		h = SGFX.gui.iHeight('advisor_stratmap')
		x = SPQR.SCREEN_WIDTH - (w + 5)
		y = SPQR.MENU_Y_OFFSET + 13
		advisor = SWIDGET.CImage(x, y, w, h, 'advisor_stratmap')
		advisor.callbacks.mouse_ldown = SEVENT.notYetCoded
		advisor.active = True
		bwindow.addWidget(advisor)				
	...
	def addKeys(self):
		...
		## allow map zoom with keys
		SGFX.gui.keyboard.addKey(K_KP_PLUS, SEVENT.keyZoomIn)
		SGFX.gui.keyboard.addKey(K_KP_MINUS, SEVENT.keyZoomOut)
		...
		SGFX.gui.keyboard.addKey(K_F5, SEVENT.menuHelpStatistics)
		SGFX.gui.keyboard.addKey(K_a, SEVENT.menuHelpAbout, KMOD_LCTRL)
		SGFX.gui.keyboard.addKey(K_F1, SEVENT.menuHelpHelp)
		SGFX.gui.keyboard.addKey(K_F2, SEVENT.menuEmpireCouncil)
		SGFX.gui.keyboard.addKey(K_F3, SEVENT.menuEmpireMilitary)
